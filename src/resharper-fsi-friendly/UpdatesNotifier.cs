...
namespace CitizenMatt.ReSharper.FsiFriendly
{
  ...
  public class UpdatesNotifier
  {
    public UpdatesNotifier(Lifetime lifetime, UpdatesManager updatesManager)
    {
      ...
      category.CustomizeLocalEnvironmentInfo.Advise(lifetime, args =>
      {
        ...
        // not. The only reason it wouldn't be is if someone has got hold of the "fsi-friendly"
    ...
    private static void RemoveStaleUpdateNotification(UpdatesCategory category)
    {
      ...
      var updateInfo =
        category.UpdateInfos.FirstOrDefault(container => new Version(container.Data.ProductVersion) <= thisVersion);
      if (updateInfo != null)
  ...
  {
    [XmlElement] public UpdateLocalEnvironmentInfoVs LocalEnvironment = new UpdateLocalEnvironmentInfoVs();

    [XmlElement] public UpdateLocalEnvironmentInfo.VersionSubInfo PluginVersion =
      new UpdateLocalEnvironmentInfo.VersionSubInfo();
  }
}